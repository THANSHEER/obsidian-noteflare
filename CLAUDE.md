...
1. **Publish** — turns the vault (or a folder / single page) into a free website. It creates a GitHub repo in the user's account, uploads transformed notes plus a tiny build manifest, and the site is built and served by **either Cloudflare Pages or GitHub Actions → GitHub Pages** (the user's choice per site).
...
**Two deploy targets, chosen per site (`SiteProfile.deployTarget`).** `'cloudflare'` or `'github-actions'`. The effective target (`Publisher.deployTarget`) defaults to `cloudflare` if a Cloudflare token is present, else `github-actions`. The content upload is identical for both; only the post-upload steps differ:
- **cloudflare:** after the commit, `Publisher` calls `enableDeployment` → `configureBuild` (self-heal) → `triggerDeployment`.
- **github-actions:** `Publisher` additionally commits `.github/workflows/deploy.yml` (the `GITHUB_ACTIONS_WORKFLOW` constant in `constants.ts`); GitHub Pages builds and deploys on push. No Cloudflare calls. **Unpublish is a manual step** in GitHub Pages settings for this target (`Publisher.unpublish` only toggles Cloudflare).
